...
import { Search, Info, Upload, FileText, CheckCircle2, ArrowRight, ArrowLeft, Loader2, Plus, Trash2 } from 'lucide-react';
...
export function LandingPage({ onRunAnalysis }: LandingPageProps) {
  ...
  useEffect(() => {
    ...
    // Use setTimeout to avoid the current click from immediately closing the menu
            ...
            {step === 2 && (
              ...
              >
                <div className="space-y-4">
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg border border-gray-200 space-y-4 overflow-visible">
                          ...
                          <div className="absolute top-full left-0 mt-1.5 z-50 bg-white rounded-lg shadow-lg border border-gray-200 min-w-[200px] py-1 max-h-[200px] overflow-y-auto">
                            {AVAILABLE_FILTERS.filter(f => !(f.key in hardFilters)).map((f) => (
                              <button
                                key={f.key}
                                onClick={() => {
                                  setHardFilters(prev => ({ ...prev, [f.key]: f.defaultValue }));
                                  setShowAddFilter(false);
                                }}
